...
namespace FuelModelComparer_BlazorFrontEnd.Components.Pages
{
    public partial class Home : Microsoft.AspNetCore.Components.ComponentBase
    {
        ...
        private readonly MudBlazor.ChartOptions _options = new();

        public int YAxisTickInterval { get; set; } = 10;  // Default tick interval
        ...
        private bool IsFirstLoad { get; set; } = false;
        private async Task btnLoadData_OnClick()
        {
            await LoadData();
            IsFirstLoad = true;
        }

        private async Task OnModelsChanged()
        {
            if (IsFirstLoad)
            {
                await LoadData();
        ...
        //        var selectedModels = new List<string?>
        //{
        //    selectedFuelModels1, selectedFuelModels2, selectedFuelModels3, selectedFuelModels4,
        //    selectedFuelModels5, selectedFuelModels6, selectedFuelModels7, selectedFuelModels8
        //};
        ...
        //        // Use the slider value for Y-axis tick frequency
        //        double interval = YAxisTickInterval;

        //        // Force the Y-axis to have more labels by modifying the dataset
        //        List<double> tickLabels = new();
        //        for (double i = globalMin; i <= globalMax; i += interval)
        //        {
        //            tickLabels.Add(i);
        //        }
        ...
        //                if (filteredData.Any())
        //                {
        //                    double minValue = (double)filteredData.Min();
        //                    double maxValue = (double)filteredData.Max();
        //                    globalMin = Math.Min(globalMin, minValue);
        //                    globalMax = Math.Max(globalMax, maxValue);

        //                    // Ensure we add more points for better Y-axis labels
        //                    double[] adjustedData = filteredData.Select(f => (double)f)
        //                                                        .Concat(tickLabels)
        //                                                        .Distinct()
        //                                                        .OrderBy(x => x)
        //                                                        .ToArray();
        ...
        //        Debug.WriteLine($"Adjusted Global Min: {globalMin}, Max: {globalMax}, Interval: {interval}");

        //        // Refresh the UI when the slider changes
        //        StateHasChanged();
        //    }
